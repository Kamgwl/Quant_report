...
const exportAccountReport = async (r) => {
  try {
    if (!ExcelJS) { alert("ExcelJS library not loaded. Please check your internet connection."); return; }
    ...
    const q2Traded = months.filter(m => m.q === "q2" && (m.pnl !== 0 || m.roi !== 0)).length;
    ...
    figure("Profitable Months", `${wins} of ${traded.length}`, { fg: INK, align: "right" }, "Months closing with a positive net result");
    R++;

    // ── MONTH-BY-MONTH ──
    band(R, 2, 6, "MONTH-BY-MONTH BREAKDOWN", { size: 10, bold: true, fg: ACC, border: { bottom: { style: "medium", color: { argb: ACC } } } });
    ws.getRow(R).height = 22; R++;

    ["Period", "Net P&L", "ROI", "Cumulative FY", "Relative Size"].forEach((h, i) => {
      const left = i === 0 || i === 4;
    ...
    const totalRow = (label, pnl, roi, tone) => {
      ...
      cell(R, 5, cum, { size: 10, bold: true, numFmt: PNL_FMT, align: "right", bg: tone, border: topline, fg: cum >= 0 ? POS_FG : NEG_FG });
    ...
    months.forEach((m, i) => {
      ...
      ws.getRow(R).height = 19; R++;
      if (m.key === "jun") totalRow("Q1 Total  (Apr–Jun)", r.q1, r.q1_roi, "FFEFF4FA");
      if (m.key === "sep") totalRow("Q2 Total  (Jul–Sep)", r.q2, r.q2_roi, "FFEFF4FA");
    });
    totalRow("FY TOTAL  (YTD)", r.year, r.year_roi, "FFE3ECF7");
    R++;

    band(R, 2, 6, "Generated from the Quant Strategy dashboard  ·  Figures are net, as recorded in the source workbook  ·  Q3–Q4 pending",
      { size: 8, italic: true, fg: MUTED, indent: 1 });

    const buffer = await wb.xlsx.writeBuffer();
    const blob = new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    const slug = (r.name || r.code).replace(/[^A-Za-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    saveAs(blob, `Quant_${slug}_${r.code}_FY2026-27.xlsx`);
  } catch (error) {
...
const exportUserSheet = () => {
  ...
  if (rows.length === 1) return exportAccountReport(rows[0]);
  const label = userSearch.trim();
  const slug = label ? "_" + label.replace(/[^A-Za-z0-9]+/g, "-").replace(/^-+|-+$/g, "") : "";
  return exportRowsToExcel(rows, COLS_ACCOUNT_FULL, "User Performance",
    `Quant_UserPerformance${slug}_FY2026-27.xlsx`);
};
...
const buildPdfReport = async () => {
  ...
  try {
    ...
    if (printSel.user) {
      ...
      sectionTitle("User Performance (Q1 / Q2 / FY)" + (q ? " - filtered: " + q : ""));
            ...
            <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, overflow: "auto" }}>
                    <table style={{ width: "100%" , borderCollapse: "collapse" , fontSize: 11 }}>
                        ...
                        <tbody>
                            ...
                            <tr><td colSpan={23} style={{ padding: "22px 10px", textAlign: "center", color: MUTED_C }}>
