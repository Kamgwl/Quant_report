...
// `onClick` turns the card into a button: it grows a hover lift and a "hint ›"
// affordance in the corner. Used by the quarter tiles to open the month popup.
const StatCard = ({ label, value, sub, color, onClick, hint }) => (
<div onClick={onClick} role={onClick ? "button" : undefined} tabIndex={onClick ? 0 : undefined}
    ...
    style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "18px 22px" , borderTop: `3px
    solid ${color || ACCENT}`, cursor: onClick ? "pointer" : "default", position: "relative",
    ...
    {onClick && (
    <div className="no-print" style={{ position: "absolute", top: 14, right: 14, color: "var(--muted2)", fontSize: 15,
        lineHeight: 1 }} title={hint || "View monthly split"}>›</div>
    )}
    {onClick && hint && (
    <div className="no-print" style={{ color: color || ACCENT, fontSize: 10, fontWeight: 700, letterSpacing: 0.6,
        marginTop: 8, opacity: 0.85, textTransform: "uppercase" }}>{hint}</div>
    )}
</div>
);
...
// Quarter drill-down popup: 'q1' | 'q2' | 'fy' | null
...
const quarterDetail = useMemo(() => {
  ...
  return { q, rows, pnl, roi, peak, best, worst, pending: rows.filter(r => !r.hasData) };
}, [openQuarter, totals]); // ── Month bar data (top 20 by absolute PnL) ──
    ...
    <div style={{ background: BG, minHeight: "100vh" , fontFamily: "'DM Sans', sans-serif" , color: "var(--text)" ,
        ...
        {/* Quarter drill-down popup — monthly split behind the Q1 / Q2 / FY cards */}
        {quarterDetail && (
          <div className="no-print" onClick={() => setOpenQuarter(null)}
            ...
            <div onClick={(e) => e.stopPropagation()}
              style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 28, width: 560,
              maxWidth: "94%", maxHeight: "88vh", overflowY: "auto", boxShadow: "0 20px 45px rgba(0,0,0,0.6)",
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
                <div>
                  <h3 style={{ fontSize: 18, fontWeight: 800, color: "var(--text)", letterSpacing: -0.5 }}>
                    {quarterDetail.q.label} · Monthly Split</h3>
                  <div style={{ color: "var(--muted)", fontSize: 12, marginTop: 3 }}>{quarterDetail.q.span} · FY 2026–27</div>
                </div>
                <button onClick={() => setOpenQuarter(null)} style={{ background: "transparent", border: "none",
                  color: "var(--muted)", fontSize: 24, cursor: "pointer", lineHeight: 1, outline: "none" }}>&times;</button>
              </div>

              {/* Quarter headline */}
              <div style={{ display: "flex", alignItems: "baseline", gap: 14, flexWrap: "wrap",
                background: quarterDetail.pnl >= 0 ? "rgba(34,197,94,0.10)" : "rgba(244,63,94,0.10)",
                border: `1px solid ${quarterDetail.pnl >= 0 ? POS : NEG}`, borderRadius: 10,
                padding: "14px 18px", marginBottom: 20 }}>
                <div style={{ fontSize: 26, fontWeight: 800, fontFamily: "'DM Mono', monospace",
                  color: quarterDetail.pnl >= 0 ? POS : NEG }}>{fmtSign(quarterDetail.pnl)}</div>
                <div style={{ color: "var(--muted)", fontSize: 12, fontWeight: 700 }}>
                  ROI {fmtROI(quarterDetail.roi)} · on {fmtFund(totals.totalFund)} AUM</div>
              </div>

              {/* Month rows */}
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {quarterDetail.rows.map(m => {
                  const c = m.pnl >= 0 ? POS : NEG;
                  const pct = Math.round((Math.abs(m.pnl) / quarterDetail.peak) * 100);
                  return (
                  <div key={m.key} style={{ border: `1px solid ${BORDER}`, borderRadius: 10, padding: "12px 14px",
                    opacity: m.hasData ? 1 : 0.5 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10 }}>
                      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                        <span style={{ color: "var(--text)", fontSize: 13, fontWeight: 700 }}>{m.label}</span>
                        {m.hasData && quarterDetail.best && m.key === quarterDetail.best.key && (
                          <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: 0.8, color: POS,
                            border: `1px solid ${POS}`, borderRadius: 4, padding: "1px 5px" }}>BEST</span>)}
                        {m.hasData && quarterDetail.worst && m.key === quarterDetail.worst.key
                          && quarterDetail.best && quarterDetail.worst.key !== quarterDetail.best.key && (
                          <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: 0.8, color: NEG,
                            border: `1px solid ${NEG}`, borderRadius: 4, padding: "1px 5px" }}>WORST</span>)}
                        {!m.hasData && (
                          <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: 0.8, color: "var(--muted2)",
                            border: `1px solid ${BORDER}`, borderRadius: 4, padding: "1px 5px" }}>PENDING</span>)}
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ color: m.hasData ? c : "var(--muted2)", fontSize: 16, fontWeight: 800,
                          fontFamily: "'DM Mono', monospace" }}>{m.hasData ? fmtSign(m.pnl) : "—"}</div>
                        {m.hasData && (
                        <div style={{ color: "var(--muted2)", fontSize: 11 }}>ROI {fmtROI(m.roi)}</div>
                        )}
                      </div>
                    </div>
                    ...
                    {m.hasData && (
                    <div style={{ color: "var(--muted2)", fontSize: 11, marginTop: 7 }}>
              ...
              {quarterDetail.pending.length > 0 && (
              <div style={{ color: "var(--muted2)", fontSize: 11, marginTop: 16 }}>
                {quarterDetail.pending.map(m => m.label).join(", ")} not booked yet — shown at zero.</div>
              )}
            </div>
          </div>
        )}
