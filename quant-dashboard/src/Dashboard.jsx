...
const monthAgg = (rows, totalFund) => {
const out = {};
FY_MONTHS.forEach(m => {
  const pnl = rows.reduce((s, r) => s + (r[m.key] || 0), 0);
  out[m.key] = {
    pnl,
    roi: totalFund > 0 ? pnl / (totalFund * 1e7) : 0,
    winners: rows.filter(r => (r[m.key] || 0) > 0).length,
    losers: rows.filter(r => (r[m.key] || 0) < 0).length,
    hasData: rows.some(r => (r[m.key] || 0) !== 0),
  };
});
return out;
};

// Contents of a quarter card's dropdown, derived from a monthAgg map.
const buildQuarterDetail = (id, byMonth, totalFund) => {
if (!id) return null;
const q = QUARTER_BY_ID[id];
const rows = q.months.map(k => ({ ...FY_MONTHS.find(m => m.key === k), ...byMonth[k] }));
const pnl = rows.reduce((s, r) => s + r.pnl, 0);
const live = rows.filter(r => r.hasData);
return {
  q, rows, pnl,
  roi: totalFund > 0 ? pnl / (totalFund * 1e7) : 0,
  peak: Math.max(1, ...rows.map(r => Math.abs(r.pnl))),
  best:  live.length ? live.reduce((a, b) => (b.pnl > a.pnl ? b : a)) : null,
  worst: live.length ? live.reduce((a, b) => (b.pnl < a.pnl ? b : a)) : null,
  pending: rows.filter(r => !r.hasData),
};
};

// Latest month carrying numbers; falls back to the calendar month so a freshly
// rolled month shows as zero rather than pinning to the previous one.
const latestMonth = (byMonth) => {
const withData = [...FY_MONTHS].reverse().find(m => byMonth[m.key].hasData);
...
// Chart series hue. The app's --accent (#00e5ff) sits above the dark-mode OKLCH
// lightness band and blooms on the near-black card, so plotted marks use this
// step instead — it clears the lightness band, the chroma floor and WCAG
// contrast against BOTH the dark (#0d1526) and light (#ffffff) chart surfaces.
const SERIES = "#0ea5c9";
const POS = "var(--pos)";
const NEG = "var(--neg)";
const BG = "var(--bg)";
...
// the Excel exports and the PDF section alike.
const PERIODS = [
  { key: "apr",  label: "Apr",  full: "April",         kind: "month",   q: "q1" },
  { key: "may",  label: "May",  full: "May",           kind: "month",   q: "q1" },
  { key: "jun",  label: "Jun",  full: "June",          kind: "month",   q: "q1" },
  { key: "q1",   label: "Q1",   full: "Q1 total",      kind: "quarter", q: "q1" },
  { key: "jul",  label: "Jul",  full: "July",          kind: "month",   q: "q2" },
...
const [userSearch, setUserSearch] = useState(""); // User tab: filters the table and scopes its export
const [periodSel, setPeriodSel] = useState(ALL_PERIODS); // User tab: which months/quarters to show
const [showPeriods, setShowPeriods] = useState(false);   // period picker popover

// Close the period popover on Esc or a click outside it — same contract as the
// KPI quarter drill-downs above.
useEffect(() => {
  if (!showPeriods) return;
  const onKey = (e) => { if (e.key === "Escape") setShowPeriods(false); };
  const onDown = (e) => {
    if (e.target && e.target.closest && !e.target.closest("[data-period-filter]")) setShowPeriods(false);
  };
...
// The Cash tab keeps its own copy so the two rows never fight over one value.
const [openQuarter, setOpenQuarter] = useState(null);
const [openCashQuarter, setOpenCashQuarter] = useState(null);
const toggleQuarter = (id) => setOpenQuarter(cur => (cur === id ? null : id));
const toggleCashQuarter = (id) => setOpenCashQuarter(cur => (cur === id ? null : id));
useEffect(() => {
  if (!openQuarter && !openCashQuarter) return;
  const closeAll = () => { setOpenQuarter(null); setOpenCashQuarter(null); };
  const onKey = (e) => { if (e.key === "Escape") closeAll(); };
  const onDown = (e) => {
    if (e.target && e.target.closest && !e.target.closest("[data-kpi-card]")) closeAll();
  };
  window.addEventListener("keydown", onKey);
  document.addEventListener("mousedown", onDown);
  return () => { window.removeEventListener("keydown", onKey); document.removeEventListener("mousedown", onDown); };
}, [openQuarter, openCashQuarter]);
...
// One wide row of 23 columns reads like a database dump, not a report. When the
// User tab is narrowed to exactly one account, export a portrait statement
// instead: identity band, key figures with peer context, then a month-by-month
// table carrying a running FY total and an inline magnitude bar.
const exportAccountReport = async (r, sel) => {
  try {
    if (!ExcelJS) { alert("ExcelJS library not loaded. Please check your internet connection."); return; }

    const NAVY = "FF0D1526", CYAN = "FF00E5FF", ACC = "FF0091B3", SUB = "FFB9C6D9";
    const INK = "FF1F2A3D", MUTED = "FF6B7A90", LINE = "FFD5DEEC", BAND = "FFF3F6FB";
    const POS_BG = "FFE6F4EA", POS_FG = "FF137333";
    const NEG_BG = "FFFCE8E6", NEG_FG = "FFC5221F";
    const F = "Segoe UI";

    const wb = new ExcelJS.Workbook();
    wb.creator = "Quant Strategy Dashboard";
    const safeName = (r.name || "Account").replace(/[\\\/\?\*\[\]:]/g, "-").slice(0, 28);
    const ws = wb.addWorksheet(safeName || "Account", {
    ...
    ws.columns = [{ width: 2 }, { width: 30 }, { width: 18 }, { width: 12 }, { width: 18 }, { width: 26 }, { width: 2 }];

    const cell = (row, col, value, o = {}) => {
      const c = ws.getRow(row).getCell(col);
      if (value !== undefined) c.value = value;
      if (o.numFmt) c.numFmt = o.numFmt;
      c.font = { name: F, size: o.size || 10, bold: !!o.bold, italic: !!o.italic, color: { argb: o.fg || INK } };
      if (o.bg) c.fill = { type: "pattern", pattern: "solid", fgColor: { argb: o.bg } };
      c.alignment = { vertical: "middle", horizontal: o.align || "left", indent: o.indent || 0, wrapText: !!o.wrap };
      if (o.border) c.border = o.border;
      return c;
    };
    // Style every cell in the range first, THEN merge — ExcelJS only paints a
    // merged block solidly if the slave cells carry the fill too.
    const band = (row, c1, c2, value, o = {}) => {
      for (let c = c1; c <= c2; c++) cell(row, c, c === c1 ? value : null, o);
      ws.mergeCells(row, c1, row, c2);
      return ws.getRow(row).getCell(c1);
    };
    ...
    // ── peer context: a number means more next to the field it competes in ──
    ...
    const peerQ1 = avg(ranked, "q1_roi"), peerFY = avg(fySorted, "year_roi");

    // ── period selection ──
    const on = (k) => !sel || sel[k] !== false;
    const allOn = PERIODS.every(p => on(p.key));
    const scope = PERIODS.filter(p => on(p.key)).map(p => p.full).join("  ·  ");

    // ── month series ──
    const months = FY_MONTHS.filter(m => on(m.key)).map(m => ({
      label: m.label + " 2026", key: m.key, q: m.q,
      pnl: r[m.key] || 0, roi: r[m.key + "_roi"] || 0,
    }));
    const traded = months.filter(m => m.pnl !== 0 || m.roi !== 0);
    const wins = traded.filter(m => m.pnl > 0).length;
    const maxAbs = Math.max(1, ...traded.map(m => Math.abs(m.pnl)));
    const best = traded.slice().sort((a, b) => b.pnl - a.pnl)[0];
    ...
    // ── IDENTITY BAND ──
    band(R, 2, 6, (r.name || "").toUpperCase(), { size: 18, bold: true, fg: CYAN, bg: NAVY, indent: 1 });
    ws.getRow(R).height = 34; R++;
    band(R, 2, 6, "ACCOUNT PERFORMANCE REPORT   ·   QUANT STRATEGY FY 2026-27", { size: 9, fg: SUB, bg: NAVY, indent: 1 });
    ws.getRow(R).height = 18; R++;
    band(R, 2, 6, `Code ${r.code}    ·    ${r.strategy || "No strategy tag"}    ·    ${r.segment || "—"}    ·    Capital ₹${r.fund.toFixed(2)} Cr    ·    Generated ${today}`,
      { size: 9, fg: MUTED, bg: BAND, indent: 1 });
    ws.getRow(R).height = 20; R++;
    if (!allOn) {
      band(R, 2, 6, `Filtered report — showing ${scope} only`, { size: 9, bold: true, fg: ACC, bg: "FFE3ECF7", indent: 1 });
      ws.getRow(R).height = 18; R++;
    }
    R++;

    // ── KEY FIGURES ──
    band(R, 2, 6, "KEY FIGURES", { size: 10, bold: true, fg: ACC, border: { bottom: { style: "medium", color: { argb: ACC } } } });
    ws.getRow(R).height = 22; R++;

    const figure = (label, value, o, note) => {
      cell(R, 2, label, { size: 10, fg: INK, border: under });
      for (let c = 3; c <= 4; c++) cell(R, c, c === 3 ? value : null, { size: 11, bold: true, align: "right", border: under, ...o });
      ws.mergeCells(R, 3, R, 4);
      for (let c = 5; c <= 6; c++) cell(R, c, c === 5 ? (note || null) : null, { size: 9, fg: MUTED, italic: true, border: under, indent: 1 });
      ws.mergeCells(R, 5, R, 6);
      ws.getRow(R).height = 19; R++;
    };
    if (on("q1")) {
      figure("Q1 Net P&L  (Apr–Jun)", r.q1, { numFmt: PNL_FMT, ...pnlTone(r.q1) },
        q1Rank ? `Rank ${q1Rank} of ${ranked.length} F&O accounts trading in Q1` : "");
      figure("Q1 ROI", r.q1_roi, { numFmt: ROI_FMT, ...pnlTone(r.q1_roi) },
        `Peer average ${pctTxt(peerQ1)}  ·  ${r.q1_roi >= peerQ1 ? "above" : "below"} average`);
    }
    if (on("q2")) {
    ...
    if (on("year")) {
      figure("FY Net P&L  (YTD)", r.year, { numFmt: PNL_FMT, bold: true, ...pnlTone(r.year) },
        fyRank ? `Rank ${fyRank} of ${fySorted.length} F&O accounts trading this FY` : "");
      figure("FY ROI  (YTD)", r.year_roi, { numFmt: ROI_FMT, bold: true, ...pnlTone(r.year_roi) },
        `Peer average ${pctTxt(peerFY)}  ·  ${r.year_roi >= peerFY ? "above" : "below"} average`);
      figure("Annualised ROI", r.ann_roi, { numFmt: ROI_FMT, ...pnlTone(r.ann_roi) },
        "Q1 run-rate × 4 — a projection, not a commitment");
    }
    figure("Capital Deployed", r.fund, { numFmt: FUND_FMT, fg: INK }, "As recorded in the strategy workbook");
    if (best) figure("Best Month", best.pnl, { numFmt: PNL_FMT, ...pnlTone(best.pnl) }, `${best.label}  ·  ${pctTxt(best.roi)}`);
    if (worst && traded.length > 1) figure("Weakest Month", worst.pnl, { numFmt: PNL_FMT, ...pnlTone(worst.pnl) }, `${worst.label}  ·  ${pctTxt(worst.roi)}`);
    if (traded.length) figure("Profitable Months", `${wins} of ${traded.length}`, { fg: INK, align: "right" }, "Months closing with a positive net result");
    R++;

    // ── MONTH-BY-MONTH ──
    const allMonthsOn = FY_MONTHS.every(m => on(m.key));
    band(R, 2, 6, allMonthsOn ? "MONTH-BY-MONTH BREAKDOWN" : "SELECTED PERIODS", { size: 10, bold: true, fg: ACC, border: { bottom: { style: "medium", color: { argb: ACC } } } });
    ws.getRow(R).height = 22; R++;

    ["Period", "Net P&L", "ROI", allMonthsOn ? "Cumulative FY" : "Cumulative", months.length ? "Relative Size" : ""].forEach((h, i) => {
      const left = i === 0 || i === 4;
      cell(R, i + 2, h, { size: 9, bold: true, fg: CYAN, bg: NAVY, align: left ? "left" : "right", indent: left ? 1 : 0 });
    });
    ws.getRow(R).height = 22; R++;

    let cum = 0;
    const totalRow = (label, pnl, roi, tone) => {
      cell(R, 2, label, { size: 10, bold: true, fg: INK, bg: tone, border: topline, indent: 1 });
      cell(R, 3, pnl, { size: 10, bold: true, numFmt: PNL_FMT, align: "right", border: topline, ...pnlTone(pnl) });
      cell(R, 4, roi, { size: 10, bold: true, numFmt: ROI_FMT, align: "right", border: topline, ...pnlTone(roi) });
      // Cumulative only means something once month rows have been written.
      cell(R, 5, months.length ? cum : "—", { size: 10, bold: true, numFmt: months.length ? PNL_FMT : null,
        align: "right", bg: tone, border: topline, fg: !months.length ? MUTED : cum >= 0 ? POS_FG : NEG_FG });
      cell(R, 6, null, { bg: tone, border: topline });
      ws.getRow(R).height = 20; R++;
    };

    months.forEach((m, i) => {
      const isTraded = m.pnl !== 0 || m.roi !== 0;
      cum += m.pnl;
      const zebra = i % 2 === 1 ? BAND : undefined;
      cell(R, 2, m.label, { size: 10, fg: isTraded ? INK : MUTED, italic: !isTraded, bg: zebra, border: under, indent: 1 });
      if (isTraded) {
        ...
        cell(R, 6, "█".repeat(width), { size: 10, align: "left", bg: zebra, fg: m.pnl >= 0 ? POS_FG : NEG_FG, indent: 1 });
      } else {
        cell(R, 3, "—", { size: 10, align: "right", fg: MUTED, bg: zebra, border: under });
        cell(R, 4, "—", { size: 10, align: "right", fg: MUTED, bg: zebra, border: under });
        cell(R, 5, "—", { size: 10, align: "right", fg: MUTED, bg: zebra, border: under });
        cell(R, 6, "not traded", { size: 9, italic: true, fg: MUTED, bg: zebra, indent: 1 });
      }
      ws.getRow(R).height = 19; R++;
      // Close a quarter once its last selected month has been written.
    ...
    const span = allOn ? "" : "_" + PERIODS.filter(p => on(p.key)).map(p => p.label).join("-");
    saveAs(blob, `Quant_${slug}_${r.code}${span}_FY2026-27.xlsx`);
  } catch (error) {
    console.error("Account report export failed", error);
    alert("An error occurred during export: " + error.message);
  }
};
...
const exportUserSheet = () => {
  ...
  // A single match gets the portrait statement; several still get the grid.
  if (rows.length === 1) return exportAccountReport(rows[0], periodSel);
  const label = userSearch.trim();
  const slug = label ? "_" + label.replace(/[^A-Za-z0-9]+/g, "-").replace(/^-+|-+$/g, "") : "";
  const span = allPeriodsOn ? "" : "_" + activePeriods.map(p => p.label).join("-");
  return exportRowsToExcel(rows, userExportCols, "User Performance",
    `Quant_UserPerformance${slug}${span}_FY2026-27.xlsx`);
};
...
const buildPdfReport = async () => {
  ...
  try {
    ...
    if (printSel.user) {
      // Mirror the User tab's search: the report covers exactly the rows on screen.
      const q = userSearch.trim();
      // Portrait A4 runs out of width past ~4 periods, so a wider pick falls
      // back to the roll-ups and says so rather than printing an unreadable grid.
      const wide = activePeriods.length > 4;
      const cols = wide ? activePeriods.filter(p => p.kind !== "month") : activePeriods;
      sectionTitle("User Performance" + (cols.length ? " (" + cols.map(p => p.label).join(" / ") + ")" : "")
        + (q ? " - filtered: " + q : ""));
      if (wide) {
        doc.setFont("helvetica", "italic"); doc.setFontSize(8); doc.setTextColor(120, 133, 152);
        doc.text("Month detail omitted here for width - the Excel export carries every selected period.", M, cursorY);
        cursorY += 14;
      }
      addTable(["#", "Code", "Name", "Strategy", "Fund", ...cols.flatMap(p => [p.label + " P&L", p.label + " ROI"])],
        userFiltered.map((r, i) => [String(userRank.get(r.code) || i + 1), r.code, r.name, r.strategy || "-", r.fund.toFixed(2),
          ...cols.flatMap(p => [pnlStr(r[p.key]), roiStr(r[p.key + "_roi"])])]),
        { 2: { cellWidth: 78 } });
    }
...
const totals = useMemo(() => {
  ...
  // Per-month roll-up keyed by month key — drives the quarter dropdown and the
...
const currentMonth = useMemo(() => latestMonth(totals.byMonth), [totals]);
const quarterDetail = useMemo(
  () => buildQuarterDetail(openQuarter, totals.byMonth, totals.totalFund),
  [openQuarter, totals]);
...
const quarterDropdown = quarterDetail
  ...
  : null; // ── Month bar data (top 20 by absolute PnL) ──
    ...
    // User tab search — matches code, name or strategy. Drives both the table
    // rows and "Export User Sheet", so the export always mirrors what is shown.
    const userQuery = userSearch.trim().toLowerCase();
    const userFiltered = useMemo(() => !userQuery ? ranked : ranked.filter(r =>
        (r.code || "").toLowerCase().includes(userQuery) ||
        (r.name || "").toLowerCase().includes(userQuery) ||
        (r.strategy || "").toLowerCase().includes(userQuery)), [ranked, userQuery]);

    // Keep the "#" column showing the account's true Q1-ROI rank while filtered.
    const userRank = useMemo(() => new Map(ranked.map((r, i) => [r.code, i + 1])), [ranked]);

    // ── Period filter (User tab) ──
    const activePeriods = useMemo(() => PERIODS.filter(p => periodSel[p.key]), [periodSel]);
    const allPeriodsOn = activePeriods.length === PERIODS.length;
    const setPeriods = (pred) => setPeriodSel(Object.fromEntries(PERIODS.map(p => [p.key, pred(p)])));
    const togglePeriod = (k) => setPeriodSel(s => ({ ...s, [k]: !s[k] }));

    // Excel columns follow the picker: identity stays, period pairs come and go.
    const userExportCols = useMemo(() => [
      ...COLS_ACCOUNT_FULL.slice(0, 4),
      ...activePeriods.flatMap(p => [
        { key: p.key, label: `${p.label} P&L`, align: "right", numFmt: PNL_FMT, colorCode: true },
        { key: p.key + "_roi", label: `${p.label} ROI`, align: "right", numFmt: ROI_FMT, colorCode: true },
      ]),
    ], [activePeriods]);
    ...
    // Same set ordered by FY ROI, for the Cash tab's full-year ranking.
    const cashRankedFY = useMemo(() => [...cashRanked]
    .sort((a, b) => b.year_roi - a.year_roi), [cashRanked]);
    ...
    // plotting it at zero would claim a 0.00% return that was never earned.
    const roiTrend = useMemo(() => FY_MONTHS.map(m => ({
      month: m.short,
      label: m.label,
      hasData: totals.byMonth[m.key].hasData,
      roi: totals.byMonth[m.key].hasData ? totals.byMonth[m.key].roi : null,
      pnl: totals.byMonth[m.key].pnl,
    })), [totals]);

    const roiStats = useMemo(() => {
      const live = roiTrend.filter(d => d.hasData);
      if (!live.length) return null;
      return {
        best: live.reduce((a, b) => (b.roi > a.roi ? b : a)),
        worst: live.reduce((a, b) => (b.roi < a.roi ? b : a)),
        avg: live.reduce((s, d) => s + d.roi, 0) / live.length,
        months: live.length,
        pending: roiTrend.filter(d => !d.hasData).map(d => d.label),
      };
    }, [roiTrend]);

    // 8px markers with a 2px surface ring, and a direct label on the best and
    // worst months only — a number on every point reads as noise.
    const RoiDot = (props) => {
      const { cx, cy, payload } = props;
      if (cx == null || cy == null || !payload || !payload.hasData) return null;
      const isBest  = roiStats && payload.month === roiStats.best.month;
      const isWorst = roiStats && payload.month === roiStats.worst.month;
      const mark = isBest || isWorst;
      return (
        <g>
          <circle cx={cx} cy={cy} r={mark ? 5 : 4} fill={SERIES} stroke={CARD} strokeWidth={2} />
          {mark && (
          <text x={cx} y={isBest ? cy - 13 : cy + 21} textAnchor="middle" fill={AXIS_C}
            fontSize={11} fontWeight={700}>{fmtROI(payload.roi)}</text>
          )}
        </g>
      );
    };

    // Pairs the month's ROI with the rupee figure behind it, so the reader does
    // not have to cross-reference the P&L chart to size the move.
    const RoiTooltip = ({ active, payload }) => {
      if (!active || !payload?.length) return null;
      const d = payload[0].payload;
      const row = (k, v) => (
        <div style={{ color: MUTED_C, display: "flex", justifyContent: "space-between", gap: 16 }}>
          <span>{k}</span>
          <span style={{ color: "var(--text)", fontWeight: 700, fontFamily: "'DM Mono', monospace" }}>{v}</span>
        </div>
      );
      return (
        <div style={{ background: TIP_BG, border: `1px solid ${TIP_BORDER}`, borderRadius: 8,
          padding: "10px 14px", fontSize: 12, minWidth: 150 }}>
          <div style={{ color: "var(--text)", fontWeight: 700, marginBottom: 5 }}>{d.label}</div>
          {d.hasData
            ? <>{row("ROI", fmtROI(d.roi))}{row("Net P&L", fmtSign(d.pnl))}</>
            : <div style={{ color: "var(--muted2)" }}>Not booked yet</div>}
        </div>
      );
    };
    ...
    <div style={{ background: BG, minHeight: "100vh" , fontFamily: "'DM Sans', sans-serif" , color: "var(--text)" ,
        ...
        <div style={{ padding: "28px 32px" }}>
                ...
                <div style={{ display: "grid" , gridTemplateColumns: "1fr 1fr" , gap: 24, marginBottom: 32 }}>
                        ...
                        <ResponsiveContainer width="100%" height={220}>
                            <BarChart data={trendData} barSize={24}>
                                ...
                                <YAxis tick={{ fill: MUTED_C , fontSize: 11 }} tickFormatter={v=> fmt(v)}
                                    ...
                                    <ReferenceLine y={0} stroke={AXIS_C} strokeWidth={1} />
                    ...
                    <div id="rpt-monthly-roi" style={{ background: CARD, borderRadius: 12, padding: 20, border: `1px solid ${BORDER}` }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline",
                            ...
                            <div style={{ color: MUTED_C , fontSize: 12, fontWeight: 700 }}>MONTHLY ROI (%)</div>
                            <div style={{ color: "var(--muted2)", fontSize: 11 }}>on {fmtFund(totals.totalFund)} AUM</div>
                        </div>
                        {/* Headline reads without hovering — the tooltip enhances, never gates. */}
                        {roiStats && (
                        <div style={{ display: "flex", gap: 22, flexWrap: "wrap", margin: "12px 0 2px" }}>
                            {[["Best", roiStats.best.label, fmtROI(roiStats.best.roi)],
                              ["Worst", roiStats.worst.label, fmtROI(roiStats.worst.roi)],
                              ["Average", `${roiStats.months} months booked`, fmtROI(roiStats.avg)]].map(([k, sub, v]) => (
                            <div key={k}>
                                <div style={{ color: "var(--muted2)", fontSize: 9, fontWeight: 700, letterSpacing: 1,
                                    textTransform: "uppercase" }}>{k}</div>
                                <div style={{ color: "var(--text)", fontSize: 14, fontWeight: 800,
                                    fontFamily: "'DM Mono', monospace" }}>{v}</div>
                                <div style={{ color: "var(--muted2)", fontSize: 10 }}>{sub}</div>
                            </div>
                            ))}
                        </div>
                        )}
                        <ResponsiveContainer width="100%" height={218}>
                            <AreaChart data={roiTrend} margin={{ top: 20, right: 14, left: 0, bottom: 0 }}>
                                <defs>
                                    <linearGradient id="roiWash" x1="0" y1="0" x2="0" y2="1">
                                        <stop offset="0%" stopColor={SERIES} stopOpacity={0.20} />
                                ...
                                <YAxis tick={{ fill: MUTED_C , fontSize: 11 }} tickFormatter={v=> `${(v*100).toFixed(2)}%`}
                                    ...
                                    <Tooltip content={<RoiTooltip />} cursor={{ stroke: MUTED_C, strokeWidth: 1 }} />
                                    {roiStats && (
                                    <ReferenceLine y={roiStats.avg} stroke={MUTED_C} strokeDasharray="4 4"
                                        label={{ value: `avg ${fmtROI(roiStats.avg)}`, position: "insideTopRight",
                                        fill: MUTED_C, fontSize: 10, fontWeight: 700 }} />
                                    )}
                                    <ReferenceLine y={0} stroke={AXIS_C} strokeWidth={1} />
                                    <Area type="monotone" dataKey="roi" name="ROI" stroke={SERIES} strokeWidth={2}
                                        strokeLinecap="round" strokeLinejoin="round" fill="url(#roiWash)"
                                        baseValue={0} connectNulls={false} isAnimationActive={false}
                                        dot={<RoiDot />} activeDot={{ r: 6, fill: SERIES, stroke: CARD, strokeWidth: 2 }} />
                            </AreaChart>
                        </ResponsiveContainer>
                        {roiStats && roiStats.pending.length > 0 && (
                        <div style={{ color: "var(--muted2)", fontSize: 10, marginTop: 6 }}>
                            {roiStats.pending.join(", ")} not booked yet — the line stops rather than plotting 0.00%.
                        </div>
                        )}
                    </div>
                </div>
            ...
            <div className="print-section">
                ...
                {/* Search — filters the table below and scopes the Export User Sheet button */}
                <div className="no-print" style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 14 }}>
                    <div style={{ position: "relative", flex: "0 1 340px", minWidth: 220 }}>
                        <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)",
                            color: MUTED2_C, fontSize: 13, pointerEvents: "none" }}>&#128269;</span>
                        <input
                            type="text"
                            value={userSearch}
                            onChange={e => setUserSearch(e.target.value)}
                            onKeyDown={e => { if (e.key === "Escape") setUserSearch(""); }}
                            placeholder="Search user by name, code or strategy…"
                            style={{ width: "100%", padding: "9px 34px 9px 34px", borderRadius: 8,
                                border: `1px solid ${BORDER}`, background: "var(--panel)", color: "var(--text)",
                                fontFamily: "'DM Sans', sans-serif", fontSize: 12, outline: "none" }} />
                        {userSearch && (
                        <button onClick={() => setUserSearch("")} title="Clear search"
                            style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)",
                                background: "none", border: "none", color: MUTED_C, cursor: "pointer",
                                fontSize: 15, lineHeight: 1, padding: 2 }}>&times;</button>
                        )}
                    </div>
                    {/* Period picker — scopes the table, the Excel export and the PDF section */}
                    <div data-period-filter style={{ position: "relative" }}>
                        <button onClick={() => setShowPeriods(v => !v)}
                            style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 14px", borderRadius: 8,
                                border: `1px solid ${showPeriods || !allPeriodsOn ? ACCENT : BORDER}`,
                                background: "var(--panel)", color: allPeriodsOn ? "var(--text)" : ACCENT,
                                fontFamily: "'DM Sans', sans-serif", fontSize: 12, fontWeight: 700,
                                cursor: "pointer", outline: "none", whiteSpace: "nowrap" }}>
                            <span>&#128197; Periods</span>
                            <span style={{ color: MUTED_C, fontWeight: 500 }}>
                                {allPeriodsOn ? "All" : `${activePeriods.length} of ${PERIODS.length}`}
                            </span>
                            <span style={{ color: MUTED_C, fontSize: 9 }}>&#9660;</span>
                        </button>
                        {showPeriods && (
                        <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, zIndex: 60, width: 302,
                            background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 14,
                            boxShadow: "0 18px 40px rgba(0,0,0,0.45)" }}>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12,
                                paddingBottom: 12, borderBottom: `1px solid ${BORDER}` }}>
                                {[
                                { label: "All", pred: () => true },
                                { label: "Months only", pred: p => p.kind === "month" },
                                { label: "Totals only", pred: p => p.kind !== "month" },
                                { label: "Q1", pred: p => p.q === "q1" },
                                { label: "Q2", pred: p => p.q === "q2" },
                                { label: "FY", pred: p => p.key === "year" },
                                ].map(preset => (
                                <button key={preset.label} onClick={() => setPeriods(preset.pred)}
                                    style={{ padding: "5px 10px", borderRadius: 6, fontSize: 10, fontWeight: 700,
                                        border: `1px solid ${BORDER}`, background: "transparent", color: "var(--violet)",
                            ...
                            <div key={g.title} style={{ marginBottom: 10 }}>
                                <div style={{ color: MUTED_C, fontSize: 9, fontWeight: 800, letterSpacing: 1.5,
                                    marginBottom: 6, textTransform: "uppercase" }}>{g.title}</div>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                                    {g.keys.map(k => {
                                    const p = PERIODS.find(x => x.key === k);
                                    const on = periodSel[k];
                                    return (
                                    <button key={k} onClick={() => togglePeriod(k)}
                                        style={{ padding: "6px 11px", borderRadius: 999, fontSize: 11, fontWeight: 700,
                                            border: `1px solid ${on ? ACCENT : BORDER}`,
                                            background: on ? "var(--row-alt)" : "transparent",
                                            color: on ? ACCENT : MUTED_C, cursor: "pointer", outline: "none",
                                            fontFamily: "'DM Sans', sans-serif" }}>{p.full}</button>
                                    );
                                    })}
                                </div>
                            </div>
                    ...
                    <span style={{ color: MUTED_C, fontSize: 11, fontWeight: 700, letterSpacing: 1 }}>
                        {userSearch.trim()
                            ? `${userFiltered.length} of ${ranked.length} ACCOUNTS · EXPORT COVERS THE MATCH`
                    ...
                    <table style={{ width: "100%" , borderCollapse: "collapse" , fontSize: 11 }}>
                        <thead>
                            <tr style={{ background: "var(--panel)" , borderBottom: `1px solid ${BORDER}` }}>
                                {["#", "Code", "Name", "Strategy", "Fund (Cr)"]
                                  .concat(activePeriods.flatMap(p => [`${p.label} P&L`, `${p.label} ROI`])).map(h => (
                        ...
                        <tbody>
                            ...
                            <tr><td colSpan={5 + activePeriods.length * 2} style={{ padding: "22px 10px", textAlign: "center", color: MUTED_C }}>
                                No accounts match “{userSearch.trim()}”.
                            </td></tr>
                            )}
                            {userFiltered.length > 0 && activePeriods.length === 0 && (
                            <tr><td colSpan={5} style={{ padding: "22px 10px", textAlign: "center", color: MUTED_C }}>
                                No periods selected — pick a month or quarter in the Periods filter.
                            </td></tr>
                            )}
                            {userFiltered.map((r, i) => {
                            const rowBg = i % 2 === 0 ? CARD : "var(--row-alt)";
                            return (
                            <tr key={r.code} style={{ background: rowBg, borderBottom: `1px solid ${BORDER}22` }}>
                                <td style={{ padding: "9px 10px" , color: "var(--muted2)" }}>{userRank.get(r.code) || i + 1}</td>
                                ...
                                <td style={{ padding: "9px 10px" , fontFamily: "'DM Mono', monospace" , color: "var(--axis)" }}>
                                    {r.fund}</td>
                                {activePeriods.map(p => [r[p.key], r[p.key + "_roi"]]).map(([pnl, roi], j) => (
                                <>
            ...
            {/* ── CASH / ETF / ATS ── */}
            {showSection("cash") && (() => {
            // Same population as the charts and detail table below. This used to
            // read `group === 2` (the Deepesh group), which has no Jul–Sep data
            // and is not the Cash/ETF set at all.
            const cashSection = DATA_CASH;
            ...
            const cashDropdown = cashDetail
              ? <QuarterMonths detail={cashDetail} totalFund={cashTotalFund} />
              : null;
            const cashTrend = FY_MONTHS.map(m => ({
              month: m.label, pnl: cashByMonth[m.key].pnl, roi: cashByMonth[m.key].roi,
            }));
            return (
            <div className="print-section">
                <div className="no-print" style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
                ...
                <div style={{ display: "grid" , gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))" , gap: 12,
                    ...
                    <StatCard label="Cash AUM" value={fmtFund(cashTotalFund)}
                        sub={`${cashSection.length} accounts`} color="#fbbf24" />
                    <StatCard label="Q1 Net P&L" value={fmtSign(cashQ1)} sub={`ROI ${fmtROI(cashROI(cashQ1))} · Apr–Jun`}
                        color={cashQ1>= 0 ? POS : NEG}
                        onClick={printMode ? undefined : () => toggleCashQuarter("q1")} hint="Monthly split"
                        open={openCashQuarter === "q1"} dropdown={cashDropdown} />
                    <StatCard label="Q2 Net P&L" value={fmtSign(cashQ2)} sub={`ROI ${fmtROI(cashROI(cashQ2))} · Jul–Sep`}
                    ...
                    <StatCard label={`${cashMonth.label} P&L`} value={fmtSign(cashByMonth[cashMonth.key].pnl)}
                        sub={`ROI ${fmtROI(cashByMonth[cashMonth.key].roi)} · Current month`}
                        color={cashByMonth[cashMonth.key].pnl >= 0 ? POS : NEG} />
                    {/* Browser print can't open dropdowns — lay the months out flat. */}
                    {printMode && FY_MONTHS.map(m => (
                    <StatCard key={m.key} label={`${m.label} P&L`} value={fmtSign(cashByMonth[m.key].pnl)}
                        sub={fmtROI(cashByMonth[m.key].roi)}
                        color={cashByMonth[m.key].pnl >= 0 ? POS : NEG} />
                    ))}
                </div>
                ...
                {/* Full-year bar chart */}
                {sectionHead("Cash / ETF — Best to Worst (FY YTD)")}
                <div style={{ background: CARD, borderRadius: 12, padding: 20, border: `1px solid ${BORDER}`,
                    marginBottom: 28 }}>
                    <ResponsiveContainer width="100%" height={Math.max(200, cashRankedFY.length * 38)}>
                        <BarChart data={cashRankedFY.map(r=> ({ name: r.name, pnl: r.year, roi: +(r.year_roi*100).toFixed(2), fund: r.fund, fullName: r.name,
                            ...
                            <XAxis type="number" tick={{ fill: MUTED_C , fontSize: 11 }} tickFormatter={v=> fmt(v)}
                                ...
                                <Tooltip content={({ active, payload })=> {
                                    ...
                                    <div style={{ background: TIP_BG , border: `1px solid ${TIP_BORDER}`, borderRadius:
                                        ...
                                        <div style={{ color: MUTED_C , fontSize: 11 }}>{d?.strategy || "—"} ·
                                            ₹{d?.fund}Cr</div>
                                        <div style={{ color: d?.pnl>= 0 ? POS : NEG }}>FY P&L: {fmt(d?.pnl)}</div>
                                        <div style={{ color: d?.roi>= 0 ? POS : NEG }}>FY ROI: {d?.roi?.toFixed(2)}%
                                        </div>
                                    </div>
                                    );
                                    }} />
                                    <ReferenceLine x={0} stroke={BORDER} />
                                    <Bar dataKey="pnl" name="FY P&L" radius={[0,4,4,0]}>
                                        {cashRankedFY.map((d, i) => <Cell key={i} fill={d.year>= 0 ? "#fbbf24" : NEG} />)}
                                    </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            ...
            <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 28, width: 470,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 3, marginBottom: 20 }}>
                {[
                  ...
                  { id: "user",      label: userSearch.trim()
                      ? `User Performance table (filtered: ${userFiltered.length} of ${ranked.length})`
